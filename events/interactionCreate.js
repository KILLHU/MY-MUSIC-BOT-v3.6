...
return interaction?.reply({ content: "Rate Limited.", ephemeral: true })
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            return interaction?.reply({ content: '🔴อัตราจำกัด'.replace("{channel_filter}", channel_filter), ephemeral: true }).catch(e => { })
            }
        }
            }
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `❌️เข้าช่องเสียงก่อน`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `🔴ต้องอยู่ในช่องเดียวกัน!!`, ephemeral: true }).catch(e => { })
            }
            }
            }
