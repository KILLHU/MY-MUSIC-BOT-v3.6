...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    try {
      if (!queue) {
        return interaction.reply({ content: '⚠️คิวว่าง!!', ephemeral: true });
      }

      if (!queue.paused) {
        return interaction.reply({ content: '⚠️ไม่มีการหยุดเพลงชั่วคราว!!', ephemeral: true });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(success ? '**ดนตรีกลับมามีชีวิตชีวาอีกครั้ง!!**' : '❌ Error: Unable to resume song')
